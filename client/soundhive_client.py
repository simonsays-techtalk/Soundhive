...
LEARN_BUFFER_TIMEOUT = 10     # Timeout in seconds for buffering learn commands
...
pending_learn_timestamp = None  # Timestamp for the last pending learn update
pending_learn_items = []        # List of learned facts pending commit

# --- Load Config (Plain JSON) & Reload ---
def load_config():
    try:
...
def reload_config():
    """Reload configuration from file and update globals."""
    ...
    global LEARN_COMMAND, FORGET_COMMAND, COMMIT_CODE, ACTIVE_TIMEOUT, LLM_URI
    config = load_config()
    ...
    global WS_API_URL, TTS_API_URL
    WS_API_URL = f"{HA_BASE_URL}/api/websocket"
    TTS_API_URL = f"{HA_BASE_URL}/api/tts_get_url"
    _LOGGER.info("Configuration reloaded: TTS_ENGINE=%s, TOKEN=%s, VOLUME_SETTING=%s, RMS_THRESHOLD=%s, HA_BASE_URL=%s", 
                 TTS_ENGINE, TOKEN, VOLUME_SETTING, RMS_THRESHOLD, HA_BASE_URL)

# Initial load and ensure unique_id
reload_config()
...
# Registration is now handled by HA via its integration.
...
# Dummy implementation of process_llm_command.
...
# --- Dummy LLM Command Processor ---
async def process_llm_command(text):
    # Dummy implementation: simply echo the text.
    return f"Echo: {text}"
